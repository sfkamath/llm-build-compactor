...
public class BuildOutputSpy extends AbstractEventSpy {
    ...
    private String previousRedirectTestOutput;
    ...
    private void emitSummary() {
        if (session == null || originalOut == null) return;

        // Try to get configuration from the project properties (allows POM <properties> or plugin <configuration>)
        ...
        List<String> includePackages = includePackagesProp.isEmpty() ? 
                List.of() : List.of(includePackagesProp.split(","));
        ...
        if (projects != null) {
            for (MavenProject project : projects) {
                ...
                if (targetDir.exists()) {
                    TestResult result = SurefireParser.parse(targetDir.toPath(), compress, includePackages);
        ...
        List<FixTarget> targets = "true".equalsIgnoreCase(showFixTargetsProp) ?
                FixTargetGenerator.generate(allErrors) : List.of();

        String showRecentChangesProp = getProperty("llmCompactor.showRecentChanges", projectProps, "true");
        List<String> recentChanges = "true".equalsIgnoreCase(showRecentChangesProp) ?
    ...
    private String getProperty(String key, Properties projectProps, String defaultValue) {
        ...
        if (projProp != null) return projProp;

        // 3. Plugin configuration (<configuration> in pom.xml)
        if (session != null && session.getTopLevelProject() != null) {
            MavenProject topProject = session.getTopLevelProject();
            org.apache.maven.model.Plugin plugin = topProject.getPlugin("io.llmcompactor:llm-compactor-maven-plugin");
            if (plugin != null) {
